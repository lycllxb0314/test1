...
**版本历史**:
- v4.2 (2025-01): **目录与章节完善**：更新目录与实际章节一致，新增设计约束、验收准则章节
...
| 约束项 | 约束内容 | 原因 |
|--------|----------|------|
| 运行环境 | Node.js 24.x | Next.js 16.x 运行要求 |
| 包管理器 | pnpm（强制） | 项目规范，禁止npm/yarn |
| 数据库 | Supabase PostgreSQL | 托管服务，已签约 |
| 端口 | 5000（Web服务） | 系统规范，9000端口系统保留 |
| 浏览器 | Chrome/Edge/Safari/Firefox最新两个版本 | 兼容性保证 |

### 13.2 业务约束

| 约束项 | 约束内容 |
|--------|----------|
| 用户规模 | 教师120人、学生1800人、家长约3600人，总计约5500用户 |
| 并发量 | 预计高峰期并发200-500人 |
| 数据保留 | 学生数据永久保留，毕业生数据归档 |
| 学期周期 | 每学年两个学期，课表按学期管理 |
| 上课时间 | 周一至周五，每天6节课（上午3节+下午3节） |
...
|--------|----------|
| 密码策略 | 最少8位，包含字母和数字 |
| 会话超时 | Access Token 2小时，Refresh Token 7天 |
| 敏感数据 | 手机号、身份证号需脱敏显示 |
| 数据传输 | 强制HTTPS |
| 文件上传 | 图片限制10MB，视频限制500MB |
...
| 组件库 | shadcn/ui（禁止引入其他UI库） |
| 样式方案 | Tailwind CSS（禁止内联样式） |
...
| 功能项 | 验收标准 |
|--------|----------|
| 轮播图展示 | 支持图片/视频/B站视频，自动轮播，点击可查看详情 |
| 童心教育 | 六大路径正确展示，点击可查看详情页 |
| 办学荣誉 | 荣誉列表正确展示，支持后台管理 |
| 新闻动态 | 新闻列表分页展示，点击可查看详情 |
| 公告通知 | 公告列表展示，支持置顶 |
...
| 功能项 | 验收标准 |
|--------|----------|
| 学生管理 | CRUD功能正常，支持批量操作，详情页显示完整信息 |
| 教师管理 | CRUD功能正常，工作量统计准确 |
| 班级管理 | CRUD功能正常，班级教师关系正确关联 |
| 手动排课 | 课表矩阵正确渲染，教师选择规则生效，草稿保存和发布功能正常 |
| 教室管理 | 教室CRUD正常，预约流程完整（申请→审批→通知） |
| 考试管理 | 考试CRUD正常，成绩录入和查询功能正常 |
| 新生注册 | 申请审核流程完整，同步到学生库功能正常 |

#### 14.1.3 德育管理系统

| 功能项 | 验收标准 |
|--------|----------|
| 习惯养成 | 八大类别配置正确，打卡、审核、评优流程完整 |
| 德育活动 | 活动发布、参与、评审流程完整 |
...
| 功能项 | 验收标准 |
|--------|----------|
| 请假申请 | 表单验证正确，审批人选择功能正常，会签/或签逻辑正确 |
| 请假审批 | 审批通过/驳回功能正常，多审批人会签/或签逻辑正确 |
| 调课处理 | 年段长可查看调课任务，安排代课教师功能正常 |
| 教室预约 | 课表矩阵视图正确，多选时段功能正常，预约流程完整 |
| 习惯审核 | 班主任审核家长打卡功能正常 |
...
| 童心教育管理 | 六大路径配置正确 |
| 办学荣誉管理 | CRUD功能正常 |
| 公告新闻管理 | 发布流程完整，定时发布功能正常 |
| 文件上传 | 图片/视频上传成功，URL正确返回 |
...
| 指标 | 验收标准 | 测试方法 |
|------|----------|----------|
| 首屏加载 | < 3秒 | Chrome DevTools Lighthouse |
| API响应 | P95 < 500ms | 压力测试 |
| 并发支持 | 500用户同时在线 | 负载测试 |
...
| 项目 | 验收标准 |
|------|----------|
| 认证安全 | JWT Token正确生成和验证，自动刷新机制生效 |
| 权限控制 | 无权限用户无法访问受保护资源 |
| 数据脱敏 | 敏感信息正确脱敏显示 |
| SQL注入 | 所有数据库查询使用参数化，无注入风险 |
| XSS防护 | 用户输入正确转义 |
...
| 项目 | 验收标准 |
|------|----------|
| 浏览器 | Chrome、Edge、Safari、Firefox最新两个版本功能正常 |
| 响应式 | 支持1920px、1366px、768px三种分辨率 |
| 移动端 | 基础功能可使用（非核心支持） |
...
└──────────────┘    └──────────────┘    └──────────────┘    └──────────────┘
       │                   │                   │                   │
       ▼                   ▼                   ▼                   ▼
   单元测试通过        功能用例通过        系统流程通过        用户签字确认
   代码审查通过        Bug修复完成         性能达标            部署上线
```
...
| 文档 | 负责人 | 说明 |
|------|--------|------|
| 测试计划 | 测试负责人 | 测试范围、策略、资源安排 |
| 测试用例 | 测试工程师 | 详细的功能和非功能测试用例 |
| 测试报告 | 测试负责人 | 测试结果、Bug统计、风险评估 |
| 验收报告 | 项目经理 | 最终验收结论、遗留问题清单 |
| 用户手册 | 产品经理 | 系统使用说明文档 |
