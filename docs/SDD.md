...
| 学业记录 | 考试成绩、排名 | 教务系统 |
| 荣誉奖项 | 获奖记录 | 教务系统 |
| 成长档案 | 成长轨迹记录（整合教务+德育） | 教务系统 + 德育系统 |
| 习惯养成 | 综合评分、各类别得分、评价记录、习惯之星 | 德育系统 |
| 德育表现 | 行为评价统计、活动参与、综合素质评价 | 德育系统 |
...
│  └── 习惯之星记录                │  └── 家长：仅自己孩子可见         │
│                                  │                                   │
│  数据刷新：允许一定延迟           │  数据刷新：允许一定延迟            │
│  编辑权限：仅展示，不可编辑       │  编辑权限：仅展示，不可编辑        │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 数据类型 | 同步方式 | 更新频率 | 存储位置 |
|----------|----------|----------|----------|
| 习惯评价记录 | API调用获取 | 实时查询 | 德育系统 |
| 习惯之星 | API调用获取 | 实时查询 | 德育系统 |
| 德育活动参与 | API调用获取 | 实时查询 | 德育系统 |
| 行为评价统计 | API调用获取 | 实时查询 | 德育系统 |
| 德育预警 | API调用获取 | 实时查询 | 德育系统 |

**权限控制矩阵**:

| 角色 | 习惯养成数据 | 德育表现数据 | 德育预警 |
|------|-------------|-------------|----------|
| 德育主任 | ✓ 全部学生 | ✓ 全部学生 | ✓ 全部学生 |
| 班主任 | ✓ 本班学生 | ✓ 本班学生 | ✓ 本班学生 |
| 家长 | ✓ 自己孩子 | ✓ 自己孩子 | ✓ 自己孩子 |
