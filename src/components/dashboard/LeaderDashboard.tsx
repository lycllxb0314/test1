...
export interface LeaderDashboardConfig {
  /** 角色标识 */
  role: string;
  /** 角色名称 */
  roleName: string;
...
const LEADER_CONFIGS: Record<string, LeaderDashboardConfig> = {
  principal: {
    role: 'principal',
    roleName: '校长',
    department: '校长室',
    bgGradient: 'bg-gradient-to-br from-blue-50/30 via-white to-indigo-50/30',
  ...
  secretary: {
    ...
    roleName: '书记',
    department: '校长室',
    bgGradient: 'bg-gradient-to-br from-red-50/30 via-white to-rose-50/30',
  ...
  moral_vice_principal: {
    ...
    roleName: '德育副校长',
    department: '德育副校长',
    bgGradient: 'bg-gradient-to-br from-green-50/30 via-white to-emerald-50/30',
  ...
  general_vice_principal: {
    ...
    roleName: '总务副校长',
    department: '总务副校长',
    bgGradient: 'bg-gradient-to-br from-amber-50/30 via-white to-orange-50/30',
...
interface LeaderDashboardProps {
  config?: LeaderDashboardConfig;
}

export function LeaderDashboard({ config }: LeaderDashboardProps) {
  const { user } = useAuth();
  
  // 根据用户角色获取配置
  const dashboardConfig = config || LEADER_CONFIGS[user?.role || 'principal'] || LEADER_CONFIGS.principal;
  ...
  return (
    <div className={`p-6 lg:p-8 space-y-6 ${dashboardConfig.bgGradient} min-h-screen`}>
      {/* 页面标题 */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {dashboardConfig.roleName}工作台
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Badge className={`${dashboardConfig.badgeColor} text-white gap-1`}>
            <Building2 className="h-3 w-3" />
            {dashboardConfig.department}
          </Badge>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="text-sm text-gray-600 space-y-2">
                  <p>• <strong>同意</strong>：内容通过审批，将进入下一环节</p>
                  <p>• <strong>驳回</strong>：内容不通过，返回修改</p>
                  <p>• <strong>退回</strong>：退回给发起人重新提交</p>
                </CardContent>
              </Card>
            </div>
          </div>
