...
export function ComprehensiveTabContent({ profile, canViewWarnings = false }: ComprehensiveTabContentProps) {
  ...
  const overallStats = useMemo(() => {
    ...
    yearData.forEach((data, year) => {
      const honorsSummary = data.honors?.summary;
      const moral = data.moral;
      const academic = data.academic;
      
      if (honorsSummary && typeof honorsSummary === 'object' && 'total' in honorsSummary) {
        totalHonors += honorsSummary.total || 0;
      }
      if (moral) {
        totalBehaviorScore += moral.avgBehaviorScore || 0;
        totalStars += moral.totalStarCount || 0;
        totalActivities += moral.totalActivityCount || 0;
      }
      yearCount++;
      if (academic) {
        academicTrend.push({
          academicYear: year,
          avgScore: academic.avgScore || 0,
          rank: academic.rank || 0,
        });
      }
    });
        ...
        <Card className="shadow-sm border-primary/20">
          <CardContent className="pt-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold">{selectedYear === 'all' ? overallStats.totalHonors : (displayData?.honors?.summary && typeof displayData.honors.summary === 'object' ? displayData.honors.summary.total || 0 : 0)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-2">
                      ...
                      {getTrendIcon(displayData.academic?.improvement || 0)}
                    </div>
                    <div className="text-sm text-muted-foreground">
                      {(displayData.academic?.improvement || 0) > 0 ? `进步${displayData.academic?.improvement}名` : 
                       (displayData.academic?.improvement || 0) < 0 ? `退步${Math.abs(displayData.academic?.improvement || 0)}名` : '排名持平'}
                    </div>
                  </div>
                </div>
                  ...
                  {(displayData.academic?.subjectAverages || []).map((subject) => (
                    <div key={subject.subject} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {getTrendIcon(subject.trend as 'up' | 'down' | 'stable')}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {selectedYear === 'all' ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={getLevelColor(displayData.moral?.overallLevel || '合格')}>
                    {displayData.moral?.overallLevel || '合格'}
                  </Badge>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-center gap-2 p-2 bg-muted/30 rounded-lg">
                    ...
                    <span className="text-sm">习惯之星 × {displayData.moral?.totalStarCount || 0}</span>
                  </div>
                  <div className="flex items-center gap-2 p-2 bg-muted/30 rounded-lg">
                    <Users className="h-4 w-4 text-blue-500" />
                    <span className="text-sm">活动参与 {displayData.moral?.totalActivityCount || 0}次</span>
                  </div>
                </div>
              </div>
            ) : (
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            ...
            ) : displayData && displayData.timeline?.events && displayData.timeline.events.length > 0 ? (
              // 单学年时间轴
              <div className="relative max-h-[280px] overflow-y-auto">
                <div className="absolute left-3 top-0 bottom-0 w-0.5 bg-muted" />
                <div className="space-y-3 pl-8">
                  ...
                  {Array.from(yearData.values()).map((data) => (
                    <tr key={data.academicYear} className="border-b last:border-0 hover:bg-muted/30">
                      ...
                      <td className="text-center py-2 px-3">
                        <Badge variant="outline">{data.honors?.summary && typeof data.honors.summary === 'object' ? data.honors.summary.total : 0}项</Badge>
                      </td>
                      <td className="text-center py-2 px-3">{data.moral?.avgBehaviorScore || '-'}</td>
                      <td className="text-center py-2 px-3">
                        <div className="flex items-center justify-center gap-1">
                          <Star className="h-3 w-3 text-yellow-500" />
                          {data.moral?.totalStarCount || 0}
                        </div>
                      </td>
                    </tr>
                  ))}
