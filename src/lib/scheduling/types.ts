 ...
 *    - 有兼任职务：只上1个班的主科目课
 *    - 无兼任职务：带2个班的主科目课
 *    - 兼任本班：书法、德法（语文老师）、劳动（数学老师）、综合实践、校本课
 *    - 班主任班会课：1课时/周，固定周五下午最后一节
 * 3. 技能科教师：优先本年级，师资不足跨段上课
 *    - 英语教师：可跨年级段教学，课时量标准同主科（14-16节/周）
 * 4. 语文课：所有年级确保每天至少1节
 * 5. 数学课：中高年级（3-6）确保每天至少1节，低年级不强制
 * 
 * ==================== 时间安排 ====================
 * - 上午3节主科，下午综合科
 * - 第一节语数轮换
...
export const STANDARD_SUBJECTS: SubjectConfig[] = [
  ...
  },
  {
    name: '体育',
    type: 'skill',
    // 一二年级5节，三至六年级4节（满足国家标准：低年级26节，中高年级30节）
    weeklyHours: { low: 5, middle: 3, high: 4 },
    applicableGrades: [1, 2, 3, 4, 5, 6],
    preferredPeriod: 'afternoon',
  },
  {
 ...
 * - 三至六年级：30课时/周
 * 
 * 低年级（1-2）：5节/天 × 5天 = 25节，实际配置26节
 * - 语文5 + 数学4 + 书法2 + 德法2 + 劳动1 + 体育5 + 音乐2 + 美术2 + 科学1 + 班会1 + 综合实践1 = 26 ✓
 * 
 * 中年级（3-4）：6节/天 × 5天 = 30节/周
 * - 语文6 + 数学5 + 书法1 + 英语2 + 德法2 + 劳动1 + 体育3 + 音乐2 + 美术2 + 科学2 + 班会1 + 信息技术1 + 校本课1 + 综合实践1 = 30 ✓
 * 
 * 高年级（5-6）：6节/天 × 5天 = 30节/周
 * - 语文6 + 数学5 + 书法1 + 英语2 + 德法2 + 劳动1 + 体育4 + 音乐1 + 美术1 + 科学2 + 班会1 + 信息技术1 + 校本课2 + 综合实践1 = 30 ✓
 */
