 ...
 * 数学备课智能体对话页面
 * 
 * AI数学教学伙伴，深度探讨数学概念、教学设计、思维培养等
 * 支持多对话管理、多模态输入
 * 
 * 智能体理念来源：
 * - 数学学习心理学（建构主义、认知发展、情感动机）
 * - 王崧舟课堂理念（动静相生、课堂状态规律）
 * - 数学教学四维分析（本质-过程-思想-结构）
 */
...
const QUICK_STARTS = [
  { icon: Calculator, text: '我在备《分数的意义》，想和你讨论概念本质', color: 'text-blue-500' },
  { icon: Brain, text: '如何设计能引发学生深度思考的数学问题', color: 'text-purple-500' },
  { icon: Target, text: '这节课的重难点我有点拿不准，能帮我分析一下吗', color: 'text-green-500' },
  { icon: Network, text: '我想梳理这个知识点的概念网络和发展脉络', color: 'text-orange-500' },
];
...
export default function MathChatPage() {
  ...
  const loadConversations = useCallback(async () => {
    try {
      ...
      if (data.success) {
        // 过滤出数学学科的对话
        const mathConvs = data.data.filter((c: ConversationListItem) => c.subject === 'math');
        setConversations(mathConvs);
      }
    } catch (error) {
  ...
  const sendMessage = async (content: string) => {
    ...
    if (!conversationId) {
      const res = await fetch('/api/conversations', {
        ...
        body: JSON.stringify({ 
          subject: 'math',
          firstMessage: typeof messageContent === 'string' ? messageContent : content.trim()
        }),
      });
      const data = await res.json();
      if (data.success) {
      ...
      {message.role === 'assistant' && (
        <Avatar className="w-8 h-8 flex-shrink-0 overflow-hidden">
          <img 
            ...
            alt="童童" 
            className="w-full h-full object-contain bg-gradient-to-br from-green-50 to-emerald-100"
          />
        </Avatar>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b bg-background flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center overflow-hidden shadow-sm">
              <img 
                src="/tongtong-avatar.png" 
                alt="童童" 
            ...
            <div>
              <h1 className="text-lg font-bold">
                {currentConversation?.title || '童童'}
              </h1>
              <p className="text-xs text-muted-foreground">数学备课智能体 · 童心数学</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
            {messages.length === 0 && !streamingContent ? (
              <div className="flex flex-col items-center justify-start pt-8 pb-4">
                ...
                />
                <h2 className="text-2xl font-bold text-center mb-2">你好，我是童童</h2>
                <p className="text-muted-foreground text-center max-w-md mb-2">
                  一位来自"童心数学"的AI教学伙伴，很高兴能和你一起探讨数学教学。
                </p>
                <p className="text-sm text-green-600 text-center max-w-md mb-6">
                  💚 童心未泯，数学育人
                </p>
                ...
                <div className="grid grid-cols-2 gap-3 max-w-2xl w-full">
                  {QUICK_STARTS.map((item, idx) => (
                    <Card
                      key={idx}
                      className="p-4 cursor-pointer hover:shadow-md hover:border-green-200 transition-all"
