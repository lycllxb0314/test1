  ...
  async computePortrait(studentId: string) {
    ...
    if (hasRichData) {
      ...
      const aiResult = await generatePortraitAI({
        ...
        gradeLevel: latestAssessment?.grade_level ?? undefined,
        ...
        hearingLeft: latestAssessment?.hearing_left ?? undefined,
        hearingRight: latestAssessment?.hearing_right ?? undefined,
        ...
        checkupNotes: latestAssessment?.checkup_notes ?? undefined,
        // 体质测试详细数据
        lungCapacity: latestAssessment?.vital_capacity ?? undefined,
        run50m: latestAssessment?.run_50m ?? undefined,
        run50x8: latestAssessment?.run_50x8 ?? undefined,
        sitReach: latestAssessment?.sit_and_reach ?? undefined,
        ropeJump: latestAssessment?.rope_jump_1min ?? undefined,
        sitUp: latestAssessment?.sit_ups_1min ?? undefined,
      });
  ...
  async generatePrescriptionFromPortrait(studentId: string) {
    ...
    try {
      ...
      const aiResult = await generatePrescriptionAI({
        ...
        weightKg: latest?.weight_kg ?? undefined,
        bmi: latest?.bmi ?? undefined,
        bmiStatus: portrait.bmiStatus,
        fitnessLevel: portrait.fitnessLevel,
        fitnessScore: latest?.total_score ?? undefined,
        overallHealthScore: portrait.overallHealthScore,
        ...
        spineNormal: latest?.spine_normal ?? undefined,
        visionLeft: latest?.vision_left ?? undefined,
        visionRight: latest?.vision_right ?? undefined,
        hearingLeft: latest?.hearing_left ?? undefined,
        hearingRight: latest?.hearing_right ?? undefined,
        systolicBp: latest?.systolic_bp ?? undefined,
        diastolicBp: latest?.diastolic_bp ?? undefined,
        heartRate: latest?.heart_rate ?? undefined,
        lungCapacity: latest?.vital_capacity ?? undefined,
        run50m: latest?.run_50m ?? undefined,
        run50x8: latest?.run_50x8 ?? undefined,
        sitReach: latest?.sit_and_reach ?? undefined,
